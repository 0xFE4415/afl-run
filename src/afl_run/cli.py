...
from afl_run.launcher import FuzzerGroup, _handle_sleep_signal
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Disable Ctrl-Z pausing the fuzzers.",
)
@click.argument(
    "config_path",
...
def main(
    ...
    try:
        asyncio.run(_run_campaign(config, resolved, timeout, fresh, dry_run, no_sleep))
...
async def _run_campaign(
    ...
    try:
        campaign = _run_campaign_with_signals(config, resolved, fresh, timeout, dry_run, no_sleep)
        await campaign
    finally:
        for signum in SHUTDOWN_SIGNALS:
...
async def _run_campaign_with_signals(
    ...
    async with FuzzerGroup(dry_run=dry_run) as group:
        if not no_sleep:
            loop.add_signal_handler(signal.SIGTSTP, lambda: _handle_sleep_signal(group))
        try:
            ...
            async def run_campaign() -> None:
                ...
                LOGGER.info("Pause: press Ctrl-Z (sleep fuzzers)")
